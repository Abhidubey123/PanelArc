<%@ Page Language="VB" AutoEventWireup="false" CodeFile="SECTION7.aspx.vb" Inherits="SECTION7" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
   
<title>PANEL ARC</title>
<link href="css/style.css" rel="stylesheet" type="text/css" media="all" />
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
<link href='http://fonts.googleapis.com/css?family=Open+Sans' rel='stylesheet' type='text/css'>
<link rel="stylesheet" href="css/responsiveslides.css">
<link rel="shortcut icon" href="images/logo.png" >
<link href="css/bootstrap1.min.css" rel='stylesheet' type='text/css' />
<link href="css/style1.css" rel='stylesheet' type='text/css' />

<script src="js/jquery.min.js"></script>
<script src="js/responsiveslides.min.js"></script>
		  <script>
		    // You can also use "$(window).load(function() {"
			    $(function () {
			      // Slideshow 1
			      $("#slider1").responsiveSlides({
			        maxwidth: 2500,
			        speed: 600
			      });
			});
		  </script>
<!--light-box-->
<script type="text/javascript" src="js/jquery.lightbox.js"></script>
<link rel="stylesheet" type="text/css" href="css/lightbox.css" media="screen">
	<script type="text/javascript">
		$(function() {
			$('.gallery a').lightBox();
		});
   </script>
<style type="text/css">
<!--
body {
	background-color: #f0f0f0;
}
-->
</style></head>
<body>
<div class="header">
	<div class="wrap">
		<div class="header-top">
			<div class="logo">
				<a href="Index.aspx"><img src="images/logo1.png" alt=""/></a>
			</div>
			
			<div class="clear"></div>
		</div>
	</div>
	<div class="menu">
		<div class="wrap">
			<div class="top-nav">
			      <ul class="nav">
		            <li  ><a href="Index.aspx">Home</a></li>
					<li class="active" ><a href="Section1.aspx">Movies</a></li>
					<li><a href="TV Series.aspx">TV Series</a></li>
					
					
					<li><a href="Panel.aspx">Panel</a></li>
					<li><a href="Online TV.aspx">Channel</a></li>
					<li ><a href="contact.aspx">Contact</a></li>
					<div class="clear"></div>
				 </ul>
				  <div class="search">
				    		<FORM method=GET action="http://www.google.com/search">
 
<INPUT TYPE=text name=q  value="">
<INPUT TYPE=hidden name=hl value="en">
<INPUT type=submit name=btnG VALUE="">
 
</FORM>
			  </div>
					<div class="clear"></div>
			</div>
      </div>
	</div>
	
</div>
      <!--start-image-slider---->
<div class="image-slider">
		<!-- Slideshow 1 -->
		  <ul class="rslides" id="slider1">
		    <li><img src="New folder/New folder (4)/movies/banner6.jpg" alt=""></li>
		    <li><img src="New folder/New folder (4)/movies/banner7.jpg" alt=""></li>
			<li><img src="New folder/New folder (4)/movies/banner5.jpg" alt=""></li>
		    <li><img src="New folder/New folder (4)/movies/banner8.jpg" alt=""></li>
			<li><img src="New folder/New folder (4)/movies/banner1.jpg" alt=""></li>
		    <li><img src="New folder/New folder (4)/movies/banner3.jpg" alt=""></li>
			<li><img src="New folder/New folder (4)/movies/banner2.jpg" alt=""></li>
		    <li><img src="New folder/New folder (4)/movies/banner4.jpg" alt=""></li>
		  </ul>
	    <!-- Slideshow 2 -->
</div>
	<!--End-image-slider---->
	</div>
    </div>
    </div>
    </div>
	<div class="wrap">
	<div class="main">
		<div class="section group">
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/New folder (6)/IMG-20150703-WA0004.jpg" alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						                
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3 align="justify"> TERMINATOR </h3>
					
					<div class="more1">
						<a href="http://hdmoviespoint.com/terminator-genisys-movie-dvdrip-hd-free-download/" class="button">Download</a>
					</div>
				</div>
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/New folder (6)/IMG-20150703-WA0006.jpg"alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						                
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3> INCIDE OUT </h3>
					
					<div class="more1">
						<a href="https://genvideos.org/watch?v=Inside_Out_2015" class="button">Download</a>					</div>
				</div>
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/New folder (6)/IMG-20150703-WA0007.jpg" alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						               
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3> MAX </h3>
					
					<div class="more1">
						<a href="https://mega.nz/#!ekJUjKJC!Pd6W5r08g9Qh_yKZ_nrSvVZc8W7a2CFuLUtnot_VGP4" class="button">Download</a>					</div>
				</div>
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/New folder (6)/IMG-20150703-WA0008.jpg" alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						               
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3> SAN ANDREAS </h3>
					
					<div class="more1">
						<a href="http://www.sanandreasmovie.com/" class="button">Download</a>					</div>
				</div>
				<div class="clear"></div>
	  </div>
			<div class="section group">
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/New folder (6)/IMG-20150703-WA0013.jpg" alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						               
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3> TOMORROW LAND </h3>
					
					<div class="more1">
						<a href="https://torrentz.in/" class="button">Download</a>					</div>
				</div>
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/New folder (6)/IMG-20150703-WA0017.jpg" alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						               
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3> INSURGENT </h3>
					
					<div class="more1">
						<a href="https://mega.nz/#!JxBliR5I!6AkPXCYOhwkGFMgI2Pyh-k1DiwvlodmoXpz8oaV3ZwQ" class="button">Download</a>					</div>
				</div>
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/New folder (6)/IMG-20150703-WA0019.jpg" alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						                
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3> JUPITER ACENDING </h3>
					
					<div class="more1">
						<a href="https://torrentz.in/" class="button">Download</a>					</div>
				</div>
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/New folder (6)/IMG-20150704-WA0003.jpg" alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						                
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3> SPECIAL ID </h3>
					
					<div class="more1">
						<a href="https://torrentz.in/" class="button">Download</a>					</div>
				</div>
				<div class="clear"></div>
				<div class="section group">
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/numbers/(1).jpg" alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						               
						              
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3 align="justify">ENTOURAGE</h3>
					
					<div class="more1">
						<a href="https://mega.nz/#!l9V0GCqD!XJ7rVg35sZNrjqdfZItLysQfGaTcJG25HKZK2M6l8hY" class="button">Download</a>
					</div>
				</div>
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/numbers/(2).jpg"alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						            
						             
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3>PITCHES 2 </h3>
					
					<div class="more1">
						<a href="http://www.downloadplayer1.com/Glowgaze-FOX-imdb/?movie=Pitch%20Perfect%202" class="button">Download</a>					</div>
				</div>
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/numbers/(3).jpg" alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						          
						              
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3>7 MINUTES </h3>
					
					<div class="more1">
						<a href="https://mega.nz/#!WV5V2RJA!mOV4aG20dX5lVXbCyLJgBM_vlX4mSIWWb2Rw0wUXSqI" class="button">Download</a>					</div>
				</div>
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/numbers/(4).jpg" alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						              
						              
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3> SPY</h3>
					
					<div class="more1">
						<a href="https://mega.nz/#!45hHwBjB!MtEYnAyPe2eTlhY3Bnq_I_atiM_tqTsNiAJtT1K72KQ" class="button">Download</a>					</div>
				</div>
				<div class="clear"></div>
	  </div>
			<div class="section group">
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/numbers/(5).jpg" alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						                
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3> VENDETTA </h3>
					
					<div class="more1">
						<a href="https://mega.nz/#!jsBFUC6L!xuxmEtmKwju4SjjvyYTDRlUAWClnNmRGIWIJZfcJ3Tw" class="button">Download</a>					</div>
				</div>
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/numbers/(6).jpg" alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						               
						             
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3>FISHING NAKED </h3>
					
					<div class="more1">
						<a href="https://mega.co.nz/#!qIFk0ayI!q6qoQb6-dlSemcKPcvSuUDa_MZqYnkDPj7pDtsdzN7g" class="button">Download</a>					</div>
				</div>
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/numbers/(7).jpg" alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						              
						             
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3> THE FOUR WARRIORS </h3>
					
					<div class="more1">
						<a href="https://mega.nz/#!yJZzADJb!tl-W1VmDkO0MQ4tMxx1uqtITOMf_yhCbJu4taCyN7XU" class="button">Download</a>					</div>
				</div>
				<div class="col_1_of_4 span_1_of_4">
					<div class="image_grid portfolio_4col">
						    <ul style="" id="list" class="portfolio_list da-thumbs"><li>
						            <img src="New folder/numbers/(8).jpg" alt="img">
						            <article class="da-animate da-slideFromLeft" style="display: block;">
						               
						             
						            </article>
						        </li>
						    </ul>
	    		  </div>
					<h3>OUIJA</h3>
					
					<div class="more1">
						<a href="https://mega.nz/#!f9AwlLLY!l7JGtIATf7t_4mSD9YbIKVJa0WBDmFkpTogMh4e_8qE" class="button">Download</a>					</div>
				</div>
				<div class="clear"></div>
			</div>
			</div>
	  </div>
	</div>
	<p><a href="Section1.aspx"><img src="New folder/numbers/2 (2).png" width="40" height="48"></a>&nbsp;<a href="Section2.aspx"><img src="New folder/numbers/2 (1).png" width="40" height="48"></a>&nbsp;<a href="Section3.aspx"><img src="New folder/numbers/2 (3).png" width="40" height="48"></a>&nbsp;<a href="Section4.aspx"><img src="New folder/numbers/2 (4).png" width="40" height="48"></a>&nbsp;<a href="Section5.aspx"><img src="New folder/numbers/2 (5).png" width="40" height="48"></a>&nbsp;<a href="Section6.aspx"><img src="New folder/numbers/2 (6).png" width="40" height="48"></a>&nbsp;<a href="Section7.aspx"><img src="New folder/numbers/2 (7).png" width="40" height="48"></a>&nbsp;<a href="Section8.aspx"><img src="New folder/numbers/2 (8).png" width="40" height="48"></a></p>
	
<div class="footer">
			<div class="wrap">
			<div class="footer-grids">
				<div class="footer-grid">
				  <h3 align="center"><strong>follow us  </strong></h3>
					<p align="center"><a href="https://twitter.com/Ad96Dubey"><img src="images/twitter.png" width="31" height="24"></a>&nbsp;&nbsp;&nbsp;<a href="https://www.facebook.com/profile.php?id=100013359484040"><img src="images/facebook.png" width="24" height="24"></a>&nbsp; &nbsp;<a href="https://plus.google.com/u/0/+ABHISHEKDUBEYupes"><img src="images/gpluse.png" width="24" height="24" border="0"></a>&nbsp;&nbsp; <a href="https://www.linkedin.com/in/abhishek-dubey-9b223bb2?trk=hp-identity-name"><img src="images/linkedin.png" width="24" height="24" border="0"></a>&nbsp;</p>
			  </div>
				<div class="footer-grid">
					<h3 align="center"><strong>HOW TO DOWNLOAD  </strong></h3>
					<div align="center">
					  <ul>
					    <li><a href="http://www.bbc.com/news">click on a dowload button</a></li>
					    <li><a href="http://www.imdb.com/">it will redirect you to russian page  </a></li>
					    <li><a href="http://www.ea.com/pc">Click on an arrow button at the top left-corner of thAT page</a></li>
				      </ul>
				  </div>
					<ul>
				  </ul>
				</div>
				 <div class="footer-grid">
					<h3 align="center"><strong>DOWNLOADER</strong></h3>
					<div align="center">
					  <ul>
					    <li><a href="https://www.internetdownloadmanager.com/download.html">INTERNET dOWNLOAD manager  </a></li>
					    <li><a href="http://en.savefrom.net/">save from net downloader </a></li>
					    <li><a href="http://www.ytddownloader.com/">ytd downloader</a></li>
				      </ul>
				   </div>
					<ul>
				   </ul>
				</div>
				

			  <div class="footer-grid footer-lastgrid">
			    <h3 align="center"><strong>CONTACT US</strong></h3>
				  <p align="center"><a href="contact panel.html" class="style6"> ABHISHEK DUBEY</a></p>
				  <div class="footer-grid-address">
					  
						
					  <p align="center">Email:<a class="email-link" href="#"> Abhishekdubey.ad96@gmail.com</a></p>
				  </div>
			  </div>
				<div class="clear"> </div>
			</div>
			</div>
	</div>
</body>
</html>

